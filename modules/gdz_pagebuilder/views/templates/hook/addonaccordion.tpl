            ...
            {foreach from=$accordions key=index item=accordion}
                <div class="card accordion-item">
                    <div class="card-header" id="heading-{$accordion_id}-{$index}">
                        <h5 class="{if $index !== 0}collapsed{/if}" data-toggle="collapse" data-target="#accordion-{$accordion_id}-{$index}" aria-expanded="false">
                            {$accordion->title|escape:'htmlall':'UTF-8'}
                        </h5>
                    </div>
                    <div id="accordion-{$accordion_id}-{$index}" class="collapse{if $index == 0} show{/if}" aria-labelledby="heading-{$accordion_id}-{$index}" data-parent="#accordion">
