...
{block name='form_buttons'}
  {if !$form_has_continue_button}
    <button type="submit" class="btn btn-active float-xs-right">{l s='Save' d='Shop.Theme.Actions'}</button>
  ...
  {else}
    <form>
      <button type="submit" class="continue btn btn-active" name="confirm-addresses" value="1">
          {l s='Continue' d='Shop.Theme.Actions'}
      </button>
      {if $customer.addresses|count > 0}
        <a class="js-cancel-address cancel-address btn" href="{url entity='order' params=['cancelAddress' => {$type}]}">{l s='Cancel' d='Shop.Theme.Actions'}</a>
      {/if}
    </form>
  {/if}
{/block}
